...
export class TabView extends React.PureComponent<Props, States> {
	scrollRef: any
	...
	renderTab = (index: number, title: string, onPress: () => void = () => {}) => (
		<TouchableOpacity
			...
			style={[
				styles.tab,
				{ backgroundColor: this.state.selectedTab == index ? colors.white : colors.primary },
			]}
			onPress={() => {
				this.setTab(index)
				onPress()
			...
			<View style={Style.row}>
				<Text
					style={[
						styles.titleTab,
						{ color: this.state.selectedTab == index ? colors.primary : colors.white },
					]}>
					{title}
				</Text>
			</View>
	...
	render() {
		...
		return (
			<View style={[styles.container, props.style]}>
				<View style={[styles.tabBar, props.styleTab]}>
